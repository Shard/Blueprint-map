...
Map.Data.populate = function(){
	...
	$('#swatch-blocks').empty();
	$('#swatch-blocks').append('<a block-type="0"><img src="http://placehold.it/24x24" /></a>');
	for (var i=0; i < Map.Data.blockTypes.length; i++) {
		if(!Map.Data.blockTypes[i]){ continue; }
		$('#swatch-blocks').append('<a block-type="'+i+'"><img src="'+Map.Data.blockTypes[i].sprites[0].image.src+'" /></a>');
	};
	
	$('#swatch-walls').empty();
	$('#swatch-walls').append('<a wall-type="0"><img src="http://placehold.it/24x24" /></a>');
	for (var i=0; i < Map.Data.wallTypes.length; i++) {
		if(!Map.Data.wallTypes[i]){ continue; }
		$('#swatch-walls').append('<a wall-type="'+i+'"><img src="'+Map.Data.wallTypes[i].src+'" /></a>');
	};
	
	$('#swatch-liquids').empty();
	$('#swatch-liquids').append('<a liquid-type="0"><img src="http://placehold.it/24x24" /></a>');
	for (var i=0; i < Map.Data.liquidTypes.length; i++) {
		if(!Map.Data.liquidTypes[i]){ continue; }
		$('#swatch-liquids').append('<a liquid-type="'+i+'"><img src="'+Map.Data.liquidTypes[i].src+'" /></a>');
	};
	
	$('#swatch-entities').empty();
	$('#swatch-entities').append('<a liquid-type="0"><img src="http://placehold.it/24x24" /></a>');
	for (var i=0; i < Map.Data.entityTypes.length; i++) {
		if(!Map.Data.entityTypes[i]){ continue; }
		$('#swatch-entities').append('<a entity-type="'+i+'"><img src="'+Map.Data.entityTypes[i].image.src+'" /></a>');
	};
	
	$('#swatch-npcs').empty();
	$('#swatch-npcs').append('<a liquid-type="0"><img src="http://placehold.it/24x24" /></a>');
	for (var i=0; i < Map.Data.npcTypes.length; i++) {
		if(!Map.Data.npcTypes[i]){ continue; }
		$('#swatch-npcs').append('<a npc-type="'+i+'"><img src="'+Map.Data.npcTypes[i].image.src+'" /></a>');
	};
...
// Set the x,y cordiante with a type i

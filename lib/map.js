...
Map.save = function(){
	...
	for (var x=0; x < Map.blocksX; x++) {
		for (var y=0; y < Map.blocksY; y++) {
			...
			if(Map.Data.walls[x][y]){
				data.walls[x + ":" + y] = Map.Data.walls[x][y];
			}
			if(Map.Data.liquids[x][y]){
				data.liquids[x + ":" + y] = Map.Data.liquids[x][y];
			}
			if(Map.Data.npcs[x][y]){
				data.npcs[x + ":" + y] = Map.Data.npcs[x][y];
			}
			if(Map.Data.entities[x][y]){
				data.entities[x + ":" + y] = Map.Data.entities[x][y];
			}
		};
	};
	...
	data.walls = JSON.stringify(data.walls);
	data.liquids = JSON.stringify(data.liquids);
	data.npcs = JSON.stringify(data.npcs);

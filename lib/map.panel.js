...
Map.Panel.init = function(){
	...
	this.register( $('#save-map')[0], "save", 'tr', 15, 15 );
	this.register( $('#mpanel-tools')[0], "tools", "tl", 15, 15 );
	this.register( $('#mpanel-swatch')[0], "swatch", "tl", 15, 190 );
	this.register( $('#mpanel-layers')[0], 'layers', 'br', -8, 40 );
	this.register( $('#mpanel-layers-toggle')[0], 'layers-toggle', 'br', 10, 10 );
	this.update();
}
...
Map.Panel.update = function(){
	...
	_.each(this.panels, function(panel, i){
		...
		if(panel.anchor == "tr"){
			...
			mtop = Map.cHeight - ( panel.y + $(panel.element).outHeight() + this.boffset );
			mleft = panel.x;
		} else if(panel.anchor == 'br'){
			mtop = Map.cHeight - ( panel.y + $(panel.element).outerHeight() + this.boffset );
			mleft =  Map.cWidth - ( panel.x + $(panel.element).outerWidth() + this.roffset );
		} else { // tl
			mtop = panel.y + this.toffset;
